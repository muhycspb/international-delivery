...
async def get_info_by_id(parcel_id: SParcelId,
                         session_id, session):
    """Получение информации о посылке по идентификатору и сессии"""
    query = select(Parcel.parcel_name,
                   ...
                   Parcel.parcel_cost_delivery,
                           ...
                           Parcel.parcel_session_id == session_id)
...
@router.post("/")
async def info_by_id(parcel_id: SParcelId,
                     response: Response,
    ...
    session_id = await check_cookie(response=response,
                                    ...
                                    session=session)
    info = await get_info_by_id(parcel_id=parcel_id,
                                session_id=session_id,
                                session=session)
    return info
